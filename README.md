...
- **The report shows the outcome, then the luck.** No letter grade. The first line says what the player lost and whether any order did better this year. The next line replays the player's exact picks through 100 versions of the same year (same event order, fresh dice) and places the real year in that spread, so a lucky year is called lucky and a rough one rough. See `replayLuck` in `engine/scorecard.ts`. `replayGrade` still exists for the practitioner edition, which is parked: it builds and deploys at `/pro/` but is not linked from the main game.
